...
function nocurves_pointCommandsToSVGPoints(pointCommands) {
...
function nocurves_pathToPoints(segments) {
...
function nocurves_convertPathsToPolygon(svgElem) {
	console.log(svgElem);
	...
	for (var i = 0; i < paths.length; i++) {
		var path = paths[i];
		var points = nocurves_pathToPoints(path.pathSegList);
		var polygonSVGPoints = nocurves_pointCommandsToSVGPoints(points);
